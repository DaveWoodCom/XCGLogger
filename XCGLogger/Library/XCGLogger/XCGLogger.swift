...
public class XCGConsoleLogDestination : XCGLogDestinationProtocol, CustomDebugStringConvertible {
    ...
    public func processLogDetails(logDetails: XCGLogDetails) {
        ...
        if showThreadName {
            extendedDetails += "[" + (NSThread.isMainThread() ? "main" : (NSThread.currentThread().name != "" ? NSThread.currentThread().name ?? String(format:"%p", NSThread.currentThread()) : String(format:"%p", NSThread.currentThread()))) + "] "
        }
...
public class XCGFileLogDestination : XCGLogDestinationProtocol, CustomDebugStringConvertible {
    ...
    private func openFile()
    {
            ...
            let logFileDirectoryURL = originalFileStandardizedURL.URLByDeletingLastPathComponent
        {
                ...
                if  let directoryEnumerator = defaultManager.enumeratorAtURL(logFileDirectoryURL, includingPropertiesForKeys: [NSFileSize], options: [.SkipsSubdirectoryDescendants, .SkipsPackageDescendants, .SkipsHiddenFiles], errorHandler: nil)
                {
                    ...
                    while var directoryEntry = (directoryEnumerator.nextObject()) as! NSURL?
                    {
                        ...
                        if  let url = directoryEntry as? NSURL where url.pathExtension == logFileExtention,
                            let  attributes = try defaultManager.attributesOfItemAtPath(url.path!) as? [String:AnyObject],
                        ...
                        {              
                            // file path error ignored
                        }
                    }

                    var sumofsizes:UInt64   = 0
                    let reversepaths        = logfileSizeDictionary.keys.sort({ $0 > $1 })
            ...
            do {
                logFileHandle = try NSFileHandle(forWritingToURL: logFileNameURL)
